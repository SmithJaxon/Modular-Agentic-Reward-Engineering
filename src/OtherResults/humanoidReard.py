...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.root_states, self.targets, self.potentials, self.prev_potentials, self.actions, self.up_vec, self.heading_vec, self.dt)
...
def compute_reward(
    ...
    # Humanoid running reward: maximize forward progress toward target,
    # encourage upright posture and discourage excessive control effort.
    ...
    # Forward progress proxy from potential change
    progress = potentials - prev_potentials

    # Velocity alignment with target direction
    target_dir = torch.zeros_like(to_target)
    target_dir = to_target / torch.clamp(target_dist.unsqueeze(-1), min=1e-6)
    horiz_speed = torch.sum(velocity[:, :2] * target_dir[:, :2], dim=-1)

    # Uprightness and heading
    up_reward = torch.clamp(up_vec[:, 2], min=0.0, max=1.0)
    heading_reward = torch.clamp(heading_vec[:, 2], min=0.0, max=1.0)

    # Penalize large actions
    action_penalty = torch.sum(actions * actions, dim=-1)

    # Smoothly shape progress and speed
    progress_temp = 1.0
    speed_temp = 1.0
    upright_temp = 1.0

    progress_reward = torch.exp(torch.clamp(progress / progress_temp, max=10.0))
    speed_reward = torch.exp(torch.clamp(horiz_speed / speed_temp, max=10.0))
    upright_reward = torch.exp(torch.clamp((up_reward + heading_reward) * 0.5 / upright_temp, max=10.0))
