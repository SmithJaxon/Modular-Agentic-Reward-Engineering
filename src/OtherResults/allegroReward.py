...
def compute_reward(
    ...
    # Orientation error via relative quaternion (object * conj(goal))
    # We use the scalar part magnitude as a smooth proxy for rotation distance.
    rel_q = quat_mul(object_rot, quat_conjugate(goal_rot))
    quat_err = 1.0 - torch.abs(rel_q[:, 3])
    quat_temp = 0.25
    orientation_reward = torch.exp(-quat_err / quat_temp)

    # Angular velocity tracking: encourage spinning while aligning to target
    angvel_mag = torch.norm(object_angvel, dim=-1)
    angvel_temp = 1.0
    spin_reward = torch.tanh(angvel_mag / angvel_temp)

    # Penalize excessive actions for smoother control
    action_penalty = torch.sum(actions * actions, dim=-1)
    action_temp = 10.0
    action_penalty = torch.exp(-action_penalty / action_temp)

    # Final reward: prioritize orientation, then encourage motion, with small control regularization
    reward = 2.0 * orientation_reward + 0.5 * spin_reward + 0.1 * action_penalty
