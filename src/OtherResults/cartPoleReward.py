...
def compute_reward(
    ...
    # dof_pos[:, 0] = cart position
    # dof_vel[:, 0] = cart velocity
    # dof_pos[:, 1] = pole angle
    # dof_vel[:, 1] = pole angular velocity
    ...
    # Temperatures for smooth shaping
    angle_temp = torch.tensor(0.5, device=dof_pos.device)
    cart_temp = torch.tensor(1.0, device=dof_pos.device)
    vel_temp = torch.tensor(1.0, device=dof_pos.device)
    ang_vel_temp = torch.tensor(1.0, device=dof_pos.device)

    # Upright pole reward: highest when angle is near 0
    upright_reward = torch.exp(-(pole_angle * pole_angle) / (2.0 * angle_temp * angle_temp))

    # Keep cart near center to avoid drifting away
    center_reward = torch.exp(-(cart_pos * cart_pos) / (2.0 * cart_temp * cart_temp))

    # Penalize excessive motion for stability
    cart_vel_reward = torch.exp(-(cart_vel * cart_vel) / (2.0 * vel_temp * vel_temp))
    ang_vel_reward = torch.exp(-(pole_ang_vel * pole_ang_vel) / (2.0 * ang_vel_temp * ang_vel_temp))

    # Weighted sum of components
    reward = 1.5 * upright_reward + 0.3 * center_reward + 0.1 * cart_vel_reward + 0.1 * ang_vel_reward
