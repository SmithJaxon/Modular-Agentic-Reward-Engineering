...
def test_sweep_planner_uses_environment_specific_variants(tmp_path: Path) -> None:
    ...
    assert humanoid_plan.runs[1].launch_target.environment_variables["MARE_ENTROPY_SCALE"] == "1.1"
    assert humanoid_plan.runs[2].launch_target.environment_variables["MARE_VALUE_CLIP_SCALE"] == "1.1"
    ...
    assert allegro_plan.runs[1].launch_target.environment_variables["MARE_ACTION_NOISE_SCALE"] == "0.8"
    assert allegro_plan.runs[2].launch_target.environment_variables["MARE_REWARD_CLIP_SCALE"] == "0.9"
